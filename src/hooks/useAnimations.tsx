...
const PRESS_SPRING: WithSpringConfig = { duration: 200 };

const CARD_PRESS_SPRING: WithSpringConfig = { duration: 200 };
...
export function useAnimatedPress() {
  ...
  const handlePressIn = useCallback(() => {
    scale.value = withTiming(0.97, PRESS_SPRING);
    opacity.value = withTiming(0.9, { duration: 120 });
  }, [scale, opacity]);

  const handlePressOut = useCallback(() => {
    scale.value = withTiming(1, PRESS_SPRING);
...
export function useAnimatedCardPress() {
  ...
  const handlePressIn = useCallback(() => {
    scale.value = withTiming(0.98, CARD_PRESS_SPRING);
  }, [scale]);

  const handlePressOut = useCallback(() => {
    scale.value = withTiming(1, CARD_PRESS_SPRING);
  }, [scale]);
