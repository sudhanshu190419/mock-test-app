const fs = require('fs');
const code = `import React, { useMemo, useState, useCallback, useEffect } from 'react';
import {
  View,
  Text,
  ScrollView,
...
const CurriculumAccordion = React.memo(function CurriculumAccordion({
  ...
  subject: {
    ...
    iconName: string;
    accentColor: string;
    chapters: Array<{ paperId: string, name: string, isLocked: boolean, duration?: number, questions?: number }>;
  };
}): React.JSX.Element {
            ...
            >
              <View style={styles.chapterRowLeft}>
                {ch.isLocked ? (
                  ...
                  <Icon name="file-text" color={coursesDark.accentPrimary} width={16} height={16} />
                )}
                <View style={{ flex: 1 }}>
                  <Text
  ...
  return (
    <Modal transparent animationType="fade" visible>
      <Animated.View style={styles.overlayBackdrop} entering={FadeIn}>
        ...
        >
          {isProcessing && (
            <Animated.View entering={ZoomIn.springify()} style={styles.overlaySpinnerWrap}>
          ...
          <Animated.Text entering={FadeInUp.delay(100)} style={styles.checkoutTitle}>{getTitle()}</Animated.Text>
          
          {isSummary && (
            <Animated.View entering={FadeInUp.delay(150)} style={styles.checkoutDetailsBox}>
          ...
          {!isSummary && (
             <Animated.Text entering={FadeInUp.delay(150)} style={styles.overlayMessage}>{getMessage()}</Animated.Text>
          )}
          ...
          {!isFailed && (
             <Animated.View layout={LinearTransition.springify()} style={styles.checkoutPayAction}>
               <TouchableOpacity 
...
export default function ExamPackDetailScreen(): React.JSX.Element {
  ...
  const packageName = packageData?.packageInfo?.name ?? '';
  const packagePrice = packageData?.packageInfo?.price ?? 0;
  const dbPackageId = packageData?.packageInfo?.packageId ?? packageId;
  ...
  async function handleShare(): Promise<void> {
    ...
    try {
      await Share.share({
        title: packageData.packageInfo.name,
        message: \`Check out "\${packageData.packageInfo.name}" PYQs on MockPrep! 🎓\n\nPrice: \${formatPrice(packageData.packageInfo.price)}\n\nDownload now!\`,
      });
    } catch {}
  }
...
const lines = fs.readFileSync('tmp_course_ref.tsx', 'utf-8').split('\n');
const stylesStart = lines.findIndex(l => l.includes('const styles = StyleSheet.create({'));
const stylesEnd = lines.length;

if (stylesStart !== -1) {
  let stylesContent = lines.slice(stylesStart, stylesEnd).join('\n');
  
  // Need to add chapterMeta inside styles
...
$1
  ...
  fs.writeFileSync('src/screens/tests/ExamPackDetailScreen.tsx', finalCode);
  console.log('Successfully wrote new ExamPackDetailScreen.tsx');
} else {
  console.error('Could not find styles in tmp_course_ref.tsx');
}
